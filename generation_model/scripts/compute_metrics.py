...
def get_gt_crys_ori(data):
    crys_array_dict = {
        'frac_coords':data.get('graph_arrays')[0],
        'atom_types':data.get('graph_arrays')[1],
        'lengths': data.get('graph_arrays')[2],
        'angles': data.get('graph_arrays')[3]
    }
...
def main(args):
    ...
    if 'opt' in args.tasks:
        ...
        if args.gt_file != '':
            data_dict = torch.load(args.gt_file)
            gt_crys = p_map(get_gt_crys_ori, [data for data in data_dict])
        else:
