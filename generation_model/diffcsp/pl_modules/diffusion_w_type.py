...
logging.basicConfig(
    level=logging.INFO,  # 设置日志级别，这里为 INFO，意味着 INFO、WARNING、ERROR 等日志都会被输出
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',  # 设置输出格式，包含时间、模块名称、级别和日志内容
    datefmt='%Y-%m-%d %H:%M:%S'  # 时间格式
)

logger = logging.getLogger(__name__)  # 获取一个日志记录器，__name__ 表示当前模块名
...
class CSPDiffusion(BaseModule):
    ...
    def forward(self, batch):
        ...
        feature_embedding = batch.feature_embedding.view(batch_size, -1)
        ...
        gt_atom_types_onehot = F.one_hot(batch.atom_types, num_classes=MAX_ATOMIC_NUM).float()
        ...
        pred_l, pred_x, pred_t = self.decoder(feature_embedding, time_emb, atom_type_probs, input_frac_coords, input_lattice, batch.num_atoms, batch.batch)
        ...
        del pred_l, pred_x, pred_t, tar_x, sigmas_per_atom, sigmas_norm_per_atom
        torch.cuda.empty_cache()
    ...
    def sample(self, batch, diff_ratio = 1.0, step_lr = 1e-5):
        ...
        for t in tqdm(range(self.beta_scheduler.timesteps, 0, -1)):
            ...
            pred_l, pred_x, pred_t = self.decoder(feature_embedding, time_emb, t_t, x_t, l_t, batch.num_atoms, batch.batch)
            ...
            pred_l, pred_x, pred_t = self.decoder(feature_embedding, time_emb, t_t_minus_05, x_t_minus_05, l_t_minus_05, batch.num_atoms, batch.batch)
    ...
    def training_step(self, batch: Any, batch_idx: int) -> torch.Tensor:
        ...
        del loss_coord, loss_lattice, loss_type
        torch.cuda.empty_cache()
